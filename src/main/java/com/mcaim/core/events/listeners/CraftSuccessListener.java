...
import com.mcaim.core.events.CraftSuccessEvent;
...
/**
 * This class is specifically handling all of
 * the craft success event calls
 */
public class CraftSuccessListener implements Listener {
    /**
    ...
    public void onCraft(CraftItemEvent event) {
        ...
        if (amount > 0) {
            final CraftSuccessEvent successEvent = new CraftSuccessEvent(player, result, amount, CraftSuccessEvent.CraftType.CRAFTING_TABLE, inv);
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (inv.getType() == InventoryType.SMITHING) {
            ...
            if (netherIngot.getAmount() < amount) {
                ...
                // Calling the event
                final CraftSuccessEvent successEvent = new CraftSuccessEvent(player, clickedItem, clickedItem.getAmount(), CraftSuccessEvent.CraftType.SMITHING_TABLE, inv);
                Bukkit.getPluginManager().callEvent(successEvent);
        ...
        if (inv.getType() == InventoryType.ANVIL) {
            ...
            if (firstItem.hasItemMeta() && Objects.requireNonNull(firstItem.getItemMeta()).hasDisplayName() &&
                ...
                if (itemName.contains("§")) {
                    ...
                    if (!itemName.equalsIgnoreCase(result.getItemMeta().getDisplayName())) {
                        ...
                        player.updateInventory();
                        player.sendMessage(ChatColor.RED + "You can't rename this item!");
                        return;
                    }
                }
            }
            ...
            // Making sure player has enough levels before calling event
            if (player.getLevel() < anvilInventory.getRepairCost())
                return;


            // Calling craft success event
            final CraftSuccessEvent successEvent = new CraftSuccessEvent(player, result, result.getAmount(), CraftSuccessEvent.CraftType.ANVIL, inv);
            Bukkit.getPluginManager().callEvent(successEvent);
